...
static inline fix16_t fix16_lshr(fix16_t x, int y)
{
    ...
    return y < 0 ? x << -y : y >= 32 ? 0 : (x) >> y;
}

static inline fix16_t fix16_rotl(fix16_t x, int y)
{
    y &= 0x1f;
    return (x << y) | (x >> (32 - y));
}

static inline fix16_t fix16_rotr(fix16_t x, int y)
{
    y &= 0x1f;
    return (x >> y) | (x << (32 - y));
}
...
#define luai_numshl(L,a,b)	((a)<< fix16_to_int((b)))
#define luai_numshr(L,a,b)	((a)>>fix16_to_int((b)))
#define luai_numlshr(L,a,b)	(fix16_lshr((a),fix16_to_int((b))))
#define luai_numrotl(L,a,b)	(fix16_rotl((a),fix16_to_int((b))))
#define luai_numrotr(L,a,b)	(fix16_rotr((a),fix16_to_int((b))))
